...
public class mxGraphicsCanvas2D implements mxICanvas2D
{
	 ...
	 */
	protected String createHtmlDocument(String text, String align, String valign, int w, int h, boolean wrap, boolean fill, boolean clip)
	{
	...
	protected void htmlText(double x, double y, double w, double h, String str, String align, String valign, boolean wrap, String format,
			boolean fill, boolean clip, double rotation)
	{
		x += state.dx;
		y += state.dy;
		...
		if (textRenderer != null && rendererPane != null)
		{
			...
			String original = str;
			str = createHtmlDocument(str, align, valign, (fill) ? (int) Math.round(w * mxConstants.PX_PER_PIXEL) : 0, 0, wrap, fill, clip);
			...
			if (((clip || wrap) && prefWidth >= w && w > 0) || (clip && prefHeight > h && h > 0))
			{
				...
				str = createHtmlDocument(original, align, valign, cw, ch, wrap, fill, clip);
				textRenderer.setText(str);
			...
			boolean fill, boolean clip, double rotation)
	{
		...
		{
			htmlText(x, y, w, h, str, align, valign, wrap, format, fill, clip, rotation);
		}
		else
		{
			plainText(x, y, w, h, str, align, valign, wrap, format, fill, clip, rotation);
		}
	}
