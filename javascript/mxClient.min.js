...
a=a.replace(/&nbsp;/g,"&#160;");c=mxUtils.parseXml('<div xmlns="http://www.w3.org/1999/xhtml">'+a+"</div>").documentElement;d.ownerDocument.importNode!=null&&(c=d.ownerDocument.importNode(c,true));d.appendChild(c);return d},s=function(c,e,f,g){if(c!=null){if(e.clip!=null){c.setAttribute("clip-path","url(#"+e.clip+")");e.clip=null}if(o!=null){c.setAttribute("d",o.join(" "));o=null;if(d&&l){c.setAttribute("shape-rendering","crispEdges");e.strokeWidth=Math.max(1,e.strokeWidth)}}e.alpha<1&&c.setAttribute("opacity",
e.alpha);f&&(e.fill!=null||e.gradient!=null)?e.gradient!=null?c.setAttribute("fill","url(#"+e.gradient+")"):c.setAttribute("fill",e.fill.toLowerCase()):b||c.setAttribute("fill","none");if(g&&e.stroke!=null){c.setAttribute("stroke",e.stroke.toLowerCase());if(e.strokeWidth!=1){if(c.nodeName=="rect"&&d)e.strokeWidth=Math.max(1,e.strokeWidth);c.setAttribute("stroke-width",e.strokeWidth)}if(c.nodeName=="path"){e.linejoin!=null&&e.linejoin!="miter"&&c.setAttribute("stroke-linejoin",e.linejoin);if(e.linecap!=
null){f=e.linecap;f=="flat"&&(f="butt");f!="butt"&&c.setAttribute("stroke-linecap",f)}e.miterlimit!=null&&(!b||e.miterlimit!=10)&&c.setAttribute("stroke-miterlimit",e.miterlimit)}if(e.dashed){f=e.dashpattern.split(" ");if(f.length>0){for(var g=[],h=0;h<f.length;h++)g[h]=Number(f[h])*k.strokeWidth;c.setAttribute("stroke-dasharray",g.join(" "))}}}e.transform.length>0&&c.setAttribute("transform",e.transform);a.appendChild(c)}},r=function(a){return Math.round(parseFloat(a)*100)/100};return{getConverter:function(){return c},
...
c++}return c/2};
mxMedianHybridCrossingReduction.prototype.transpose=function(a,b){for(var c=true,d=0;c&&d++<10;)for(var e=a%2==1&&d%2==1,c=false,f=0;f<b.ranks.length;f++){for(var g=b.ranks[f],h=[],i=0;i<g.length;i++){var k=g[i],l=k.getGeneralPurposeVariable(f);l<0&&(l=i);h[l]=k}for(var m=l=k=null,n=null,o=null,q=null,p=null,t=null,u=null,v=null,i=0;i<g.length-1;i++){if(i==0){for(var u=h[i],k=u.getNextLayerConnectedCells(f),l=u.getPreviousLayerConnectedCells(f),o=[],q=[],s=0;s<k.length;s++)o[s]=k[s].getGeneralPurposeVariable(f+1);
for(s=0;s<l.length;s++)q[s]=l[s].getGeneralPurposeVariable(f-1)}else{k=m;l=n;o=p;q=t;u=v}v=h[i+1];m=v.getNextLayerConnectedCells(f);n=v.getPreviousLayerConnectedCells(f);p=[];t=[];for(s=0;s<m.length;s++)p[s]=m[s].getGeneralPurposeVariable(f+1);for(s=0;s<n.length;s++)t[s]=n[s].getGeneralPurposeVariable(f-1);for(var r=0,y=0,s=0;s<o.length;s++)for(var x=0;x<p.length;x++){o[s]>p[x]&&r++;o[s]<p[x]&&y++}for(s=0;s<q.length;s++)for(x=0;x<t.length;x++){q[s]>t[x]&&r++;q[s]<t[x]&&y++}if(y<r||y==r&&e){m=u.getGeneralPurposeVariable(f);
...
mxMedianHybridCrossingReduction.prototype.medianRank=function(a,b){for(var c=this.nestedBestRanks[a].length,d=[],e=[],f=0;f<c;f++){var g=this.nestedBestRanks[a][f],h=new MedianCellSorter;h.cell=g;var i;i=b?g.getNextLayerConnectedCells(a):g.getPreviousLayerConnectedCells(a);var k;k=b?a+1:a-1;if(i!=null&&i.length!=0){h.medianValue=this.medianValue(i,k);d.push(h)}else e[g.getGeneralPurposeVariable(a)]=true}d.sort(MedianCellSorter.prototype.compare);for(f=0;f<c;f++)if(e[f]==null){g=d.shift().cell;g.setGeneralPurposeVariable(a,
...
mxCoordinateAssignment.prototype.minNode=function(a){for(var b=[],c=[],d=[],e=0;e<=a.maxRank;e++){d[e]=a.ranks[e];for(var f=0;f<d[e].length;f++){var g=d[e][f],h=new WeightedCellSorter(g,e);h.rankIndex=f;h.visited=true;b.push(h);g=mxCellPath.create(g.getCoreCell());c[g]=h}}a=b.length*10;for(f=0;b.length>0&&f<=a;){var h=b.shift(),e=h.cell,i=h.weightedValue,k=parseInt(h.rankIndex),g=e.getNextLayerConnectedCells(i),l=e.getPreviousLayerConnectedCells(i),m=g.length,n=l.length,o=this.medianXValue(g,i+1),
q=this.medianXValue(l,i-1),p=m+n,t=e.getGeneralPurposeVariable(i),u=t;p>0&&(u=(o*m+q*n)/p);m=false;if(u<t-1)if(k==0){e.setGeneralPurposeVariable(i,u);m=true}else{k=d[i][k-1];t=k.getGeneralPurposeVariable(i);t=t+k.width/2+this.intraCellSpacing+e.width/2;if(t<u){e.setGeneralPurposeVariable(i,u);m=true}else if(t<e.getGeneralPurposeVariable(i)-1){e.setGeneralPurposeVariable(i,t);m=true}}else if(u>t+1)if(k==d[i].length-1){e.setGeneralPurposeVariable(i,u);m=true}else{k=d[i][k+1];t=k.getGeneralPurposeVariable(i);
t=t-k.width/2-this.intraCellSpacing-e.width/2;if(t>u){e.setGeneralPurposeVariable(i,u);m=true}else if(t>e.getGeneralPurposeVariable(i)+1){e.setGeneralPurposeVariable(i,t);m=true}}if(m){for(e=0;e<g.length;e++){i=g[e];i=mxCellPath.create(i.getCoreCell());i=c[i];if(i!=null&&i.visited==false){i.visited=true;b.push(i)}}for(e=0;e<l.length;e++){i=l[e];i=mxCellPath.create(i.getCoreCell());i=c[i];if(i!=null&&i.visited==false){i.visited=true;b.push(i)}}}h.visited=false;f++}};
...
g.getNextLayerConnectedCells(a).slice();if(i!=null){h=i.length;h=h>0?this.medianXValue(i,c):g.getGeneralPurposeVariable(a)}for(var k=0,i=-1E8,l=d[f].rankIndex-1;l>=0;){var m=mxCellPath.create(b[l].getCoreCell()),m=e[m];if(m!=null){var n=m.cell;if(m.visited){i=n.getGeneralPurposeVariable(a)+n.width/2+this.intraCellSpacing+k+g.width/2;l=-1}else{k=k+(n.width+this.intraCellSpacing);l--}}}k=0;n=1E8;for(l=d[f].rankIndex+1;l<d.length;){m=mxCellPath.create(b[l].getCoreCell());m=e[m];if(m!=null){var o=m.cell;
if(m.visited){n=o.getGeneralPurposeVariable(a)-o.width/2-this.intraCellSpacing-k-g.width/2;l=d.length}else{k=k+(o.width+this.intraCellSpacing);l++}}}if(h>=i&&h<=n)g.setGeneralPurposeVariable(a,h);else if(h<i){g.setGeneralPurposeVariable(a,i);this.currentXDelta=this.currentXDelta+(i-h)}else if(h>n){g.setGeneralPurposeVariable(a,n);this.currentXDelta=this.currentXDelta+(h-n)}d[f].visited=true}};
...
mxCoordinateAssignment.prototype.minPath=function(a,b){var c=b.edgeMapper,d;for(d in c){var e=c[d];if(!(e.maxRank-e.minRank-1<1)){for(var f=e.getGeneralPurposeVariable(e.minRank+1),g=true,h=0,i=e.minRank+2;i<e.maxRank;i++){var k=e.getGeneralPurposeVariable(i);if(f!=k){g=false;f=k}else h++}if(!g){for(var g=f=0,k=[],l=[],m=e.getGeneralPurposeVariable(e.minRank+1),i=e.minRank+1;i<e.maxRank-1;i++){var n=e.getX(i+1);if(m==n){k[i-e.minRank-1]=m;f++}else if(this.repositionValid(b,e,i+1,m)){k[i-e.minRank-
1]=m;f++}else m=k[i-e.minRank-1]=n}m=e.getX(i);for(i=e.maxRank-1;i>e.minRank+1;i--){n=e.getX(i-1);if(m==n){l[i-e.minRank-2]=m;g++}else if(this.repositionValid(b,e,i-1,m)){l[i-e.minRank-2]=m;g++}else{l[i-e.minRank-2]=e.getX(i-1);m=n}}if(g>h||f>h)if(g>=f)for(i=e.maxRank-2;i>e.minRank;i--)e.setX(i,l[i-e.minRank-1]);else if(f>g)for(i=e.minRank+2;i<e.maxRank;i++)e.setX(i,k[i-e.minRank-2])}}}};
...
mxCoordinateAssignment.prototype.setEdgePosition=function(a){var b=0;if(a.temp[0]!=101207){var c=a.maxRank,d=a.minRank;if(c==d){c=a.source.maxRank;d=a.target.minRank}for(var e=0,f=this.jettyPositions[mxCellPath.create(a.edges[0])],g=a.source.cell,h=0;h<a.edges.length;h++){var i=a.edges[h],k=this.layout.graph.view.getVisibleTerminal(i,true),l=[],m=a.isReversed;k!=g&&(m=!m);if(f!=null){var k=m?2:0,n=m?this.rankTopY[d]:this.rankBottomY[c],o=f[e*4+1+k];m&&(o=-o);n=n+o;k=f[e*4+k];this.orientation==mxConstants.DIRECTION_NORTH||
this.orientation==mxConstants.DIRECTION_SOUTH?l.push(new mxPoint(k,n)):l.push(new mxPoint(n,k))}var q=a.x.length-1,n=k=-1,o=a.maxRank-1;if(m){q=0;k=a.x.length;n=1;o=a.minRank+1}for(;a.maxRank!=a.minRank&&q!=k;q=q+n){var p=a.x[q]+b,t=(this.rankTopY[o]+this.rankBottomY[o+1])/2,u=(this.rankTopY[o-1]+this.rankBottomY[o])/2;if(m)var v=t,t=u,u=v;if(this.orientation==mxConstants.DIRECTION_NORTH||this.orientation==mxConstants.DIRECTION_SOUTH){l.push(new mxPoint(p,t));l.push(new mxPoint(p,u))}else{l.push(new mxPoint(t,
p));l.push(new mxPoint(u,p))}this.limitX=Math.max(this.limitX,p);o=o+n}if(f!=null){k=m?2:0;n=m?this.rankBottomY[c]:this.rankTopY[d];o=f[e*4+3-k];m&&(o=-o);n=n-o;k=f[e*4+2-k];this.orientation==mxConstants.DIRECTION_NORTH||this.orientation==mxConstants.DIRECTION_SOUTH?l.push(new mxPoint(k,n)):l.push(new mxPoint(n,k))}a.isReversed&&this.processReversedEdge(a,i);this.layout.setEdgePoints(i,l);b=b==0?this.parallelEdgeSpacing:b>0?-b:-b+this.parallelEdgeSpacing;e++}a.temp[0]=101207}};
