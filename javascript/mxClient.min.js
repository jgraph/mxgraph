...
k=new mxPoint(i,g)}else b.lineTo(k.x,k.y);g=k}b.lineTo(c.x,c.y);b.stroke();return true}}};this.shapes.arrow={drawShape:function(a,c,d,e){if(e){var d=mxConstants.ARROW_SPACING,f=mxConstants.ARROW_WIDTH,e=mxConstants.ARROW_SIZE,c=c.absolutePoints,g=c[0],c=c[c.length-1],h=c.x-g.x,k=c.y-g.y,i=Math.sqrt(h*h+k*k),l=i-2*d-e,e=h/i,k=k/i,i=f*k/3,f=-f*e/3,h=g.x-i/2+d*e,g=g.y-f/2+d*k,m=h+i,n=g+f,o=m+l*e,l=n+l*k,p=o+i,q=l+f,t=p-3*i,u=q-3*f;a.begin();a.moveTo(h,g);a.lineTo(m,n);a.lineTo(o,l);a.lineTo(p,q);a.lineTo(c.x-
...
isAutoAntiAlias:function(){return d},setAutoAntiAlias:function(a){d=a},isTextEnabled:function(){return e},setTextEnabled:function(a){e=a},isFoEnabled:function(){return f},setFoEnabled:function(a){f=a},save:function(){u.push(i);i=mxUtils.clone(i)},restore:function(){i=u.pop()},scale:function(a){i.scale=i.scale*a;i.strokeWidth=i.strokeWidth*a},translate:function(a,b){i.dx=i.dx+a;i.dy=i.dy+b},rotate:function(a,b,c,d,e){d=d+i.dx;e=e+i.dy;d=d*i.scale;e=e*i.scale;if(b^c){var f=b?d:0,b=b?-1:1,g=c?e:0,c=
c?-1:1;i.transform=i.transform+("translate("+r(f)+","+r(g)+")");i.transform=i.transform+("scale("+r(b)+","+r(c)+")");i.transform=i.transform+("translate("+r(-f)+" "+r(-g)+")")}i.transform=i.transform+("rotate("+r(a)+","+r(d)+","+r(e)+")")},setStrokeWidth:function(a){i.strokeWidth=a*i.scale},setStrokeColor:function(a){i.stroke=a},setDashed:function(a){i.dashed=a},setDashPattern:function(a){i.dashpattern=a},setLineCap:function(a){i.linecap=a},setLineJoin:function(a){i.linejoin=a},setMiterLimit:function(a){i.miterlimit=
a},setFontSize:function(a){i.fontSize=a},setFontColor:function(a){i.fontColor=a},setFontFamily:function(a){i.fontFamily=a},setFontStyle:function(a){i.fontStyle=a},setAlpha:function(a){i.alpha=a},setFillColor:function(a){i.fill=a;i.gradient=null},setGradient:function(a,b,c,d,e,f,k){if(a!=null&&b!=null){c=i;d=a;e=b;d.charAt(0)=="#"&&(d=d.substring(1));e.charAt(0)=="#"&&(e=e.substring(1));d=d.toLowerCase();e=e.toLowerCase();f=null;if(k==null||k==mxConstants.DIRECTION_SOUTH)f="s";else if(k==mxConstants.DIRECTION_EAST)f=
"e";else{var l=d,d=e,e=l;k==mxConstants.DIRECTION_NORTH?f="s":k==mxConstants.DIRECTION_WEST&&(f="e")}d=d+"-"+e+"-"+f;e=q[d];if(e==null){e=g("linearGradient");e.setAttribute("id",++t);e.setAttribute("x1","0%");e.setAttribute("y1","0%");e.setAttribute("x2","0%");e.setAttribute("y2","0%");k==null||k==mxConstants.DIRECTION_SOUTH?e.setAttribute("y2","100%"):k==mxConstants.DIRECTION_EAST?e.setAttribute("x2","100%"):k==mxConstants.DIRECTION_NORTH?e.setAttribute("y1","100%"):k==mxConstants.DIRECTION_WEST&&
e.setAttribute("x1","100%");k=g("stop");k.setAttribute("offset","0%");k.setAttribute("style","stop-color:"+a);e.appendChild(k);k=g("stop");k.setAttribute("offset","100%");k.setAttribute("style","stop-color:"+b);e.appendChild(k);h.appendChild(e);q[d]=e}b=e.getAttribute("id");c.gradient=b;i.fill=a}},setGlassGradient:function(){if(m==null){m=g("linearGradient");m.setAttribute("id","0");m.setAttribute("x1","0%");m.setAttribute("y1","0%");m.setAttribute("x2","0%");m.setAttribute("y2","100%");var a=g("stop");
...
false,mxConstants.DIRECTION_MASK_ALL));a=[0,0];d=[[d,g,i,l],[m,n,o,p]];for(i=0;i<2;i++){mxEdgeStyle.limits[i][1]=d[i][0]-f;mxEdgeStyle.limits[i][2]=d[i][1]-f;mxEdgeStyle.limits[i][4]=d[i][0]+d[i][2]+f;mxEdgeStyle.limits[i][8]=d[i][1]+d[i][3]+f}i=d[0][0]+d[0][2]/2-(d[1][0]+d[1][2]/2);l=d[0][1]+d[0][3]/2-(d[1][1]+d[1][3]/2);g=0;if(i<0)g=l<0?2:1;else if(l<=0){g=3;i==0&&(g=2)}l=null;b!=null&&(l=h);b=[[0.5,0.5],[0.5,0.5]];for(i=0;i<2;i++){if(l!=null){b[i][0]=(l.x-d[i][0])/d[i][2];if(b[i][0]<0.01)a[i]=
...
Math.max(m-2*f,0);mxEdgeStyle.vertexSeperations[3]=Math.max(n-2*f,0);c=[];h=[];k=[];h[0]=l>=n?mxConstants.DIRECTION_MASK_WEST:mxConstants.DIRECTION_MASK_EAST;k[0]=i>=m?mxConstants.DIRECTION_MASK_NORTH:mxConstants.DIRECTION_MASK_SOUTH;h[1]=mxUtils.reversePortConstraints(h[0]);k[1]=mxUtils.reversePortConstraints(k[0]);l=l>=n?l:n;m=i>=m?i:m;n=[[0,0],[0,0]];o=false;for(i=0;i<2;i++)if(a[i]==0){(h[i]&q[i])==0&&(h[i]=mxUtils.reversePortConstraints(h[i]));(k[i]&q[i])==0&&(k[i]=mxUtils.reversePortConstraints(k[i]));
n[i][0]=k[i];n[i][1]=h[i]}if(m>f*2&&l>f*2)if((h[0]&q[0])>0&&(k[1]&q[1])>0){n[0][0]=h[0];n[0][1]=k[0];n[1][0]=k[1];n[1][1]=h[1];o=true}else if((k[0]&q[0])>0&&(h[1]&q[1])>0){n[0][0]=k[0];n[0][1]=h[0];n[1][0]=h[1];n[1][1]=k[1];o=true}if(m>f*2&&!o){n[0][0]=k[0];n[0][1]=h[0];n[1][0]=k[1];n[1][1]=h[1];o=true}if(l>f*2&&!o){n[0][0]=h[0];n[0][1]=k[0];n[1][0]=h[1];n[1][1]=k[1]}for(i=0;i<2;i++)if(a[i]==0){(n[i][0]&q[i])==0&&(n[i][0]=n[i][1]);c[i]=n[i][0]&q[i];c[i]=c[i]|(n[i][1]&q[i])<<8;c[i]=c[i]|(n[1-i][i]&
q[i])<<16;c[i]=c[i]|(n[1-i][1-i]&q[i])<<24;(c[i]&15)==0&&(c[i]=c[i]<<8);(c[i]&3840)==0&&(c[i]=c[i]&15|c[i]>>8);(c[i]&983040)==0&&(c[i]=c[i]&65535|(c[i]&251658240)>>8);a[i]=c[i]&15;if(q[i]==mxConstants.DIRECTION_MASK_WEST||q[i]==mxConstants.DIRECTION_MASK_NORTH||q[i]==mxConstants.DIRECTION_MASK_EAST||q[i]==mxConstants.DIRECTION_MASK_SOUTH)a[i]=q[i]}i=a[0]==mxConstants.DIRECTION_MASK_EAST?3:a[0];q=a[1]==mxConstants.DIRECTION_MASK_EAST?3:a[1];i=i-g;q=q-g;i<1&&(i=i+4);q<1&&(q=q+4);q=mxEdgeStyle.routePatterns[i-
...
d.setAttribute("width",c);d.setAttribute("height",b);if(c<=this.container.offsetWidth&&this.container.clientWidth<this.container.offsetWidth){d=this.container.style.overflow;this.container.style.overflow="hidden";this.container.scrollLeft=1;this.container.style.overflow=d}}else{c=Math.ceil(c);b=Math.ceil(b);d=this.view.getDrawPane();e=this.view.getCanvas();d.style.width=c+"px";d.style.height=b+"px";if(this.minimumGraphSize!=null){c=Math.max(c,Math.ceil(this.minimumGraphSize.width*this.view.scale));
b=Math.max(b,Math.ceil(this.minimumGraphSize.height*this.view.scale))}e.style.width=c+"px";e.style.height=b+"px"}this.updatePageBreaks(this.pageBreaksVisible,c-1,b-1)}this.fireEvent(new mxEventObject(mxEvent.SIZE,"bounds",a))};
...
this.graph,a);a.shape!=null&&b.setCursor(a.shape.getCursor());var c=!this.graph.model.isEdge(a.cell)?Number(a.style[mxConstants.STYLE_ROTATION]||"0"):0;if(b.dialect==mxConstants.DIALECT_SVG){b.node.setAttribute("style","pointer-events:none;");if(c!=0){var d=a.getCenterX(),a=a.getCenterY();b.node.setAttribute("transform","rotate("+c+" "+d+" "+a+")")}}else{b.node.style.background="";if(c!=0)b.node.rotation=c}return b};mxCellHighlight.prototype.hide=function(){this.highlight(null)};
mxCellHighlight.prototype.highlight=function(a){if(this.state!=a){if(this.shape!=null){this.shape.destroy();this.shape=null}if(a!=null)this.shape=this.drawHighlight(a);this.state=a}};mxCellHighlight.prototype.destroy=function(){this.graph.getView().removeListener(this.resetHandler);this.graph.getModel().removeListener(this.resetHandler);if(this.shape!=null){this.shape.destroy();this.shape=null}};
